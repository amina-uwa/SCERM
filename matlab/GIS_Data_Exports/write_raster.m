function write_raster(outx,outy,data,savename)

outx = fliplr(outx);
outy = flipud(outy);
data = flipud(data);

raster_var = [outx(:) outy(:) data(:)];
SaveAsciiRaster_bb(raster_var,savename);

pro_name = regexprep(savename,'.asc','.prj');

fid = fopen(pro_name,'wt');
fprintf(fid,'Projection    UTM\n');
fprintf(fid,'Zone          50\n');
fprintf(fid,'Datum         WGS84\n');
fprintf(fid,'Spheroid      WGS84\n');
fprintf(fid,'Units         METERS\n');
fprintf(fid,'Zunits        NO\n');
fprintf(fid,'Yshift        10000000.0\n');
...
function SaveAsciiRaster_bb(varname, savename)
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
%                                                                 %
...
ncols = header(1,1);
nrows = header(2,1);
handle = waitbar(0, 'Saving data to ASC');
for CurrRow = 1:nrows;

...
load HSIdata_monthly.mat

vars = fieldnames(HSIdata);
...
for i = 1:length(vars)
    ...
    dates = fieldnames(HSIdata.(vars{i}));
    
    vn = vars{i};
    vn = regexprep(vn,'overall','OA');
    
    
    for j = 1:length(dates)
        theString = [theString;[vn,'_',dates{j}]];
        
        theMatrix(:,int) = HSIdata.(vars{i}).(dates{j});
 ...
 convert_2dm_to_shp_matrix('GIS/Swan_Full_EQ_Seagrass_v2.2dm','HSIdata.shp',theString,theMatrix);    

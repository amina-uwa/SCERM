function create_interpolated_BC_for_inflow_UpperSwan_2000(swan,headers,datearray)
...
filename = 'Upper_Swan_Inflow_2000.csv';
outdir = 'BCs/Flow/';
...
t_depth = swan.s616011.(varname).Depth;
tt = find(t_depth > -0.1);

[t_date,ind] = unique(swan.s616011.(varname).Date(tt));
t_data = swan.s616011.(varname).Data(tt(ind));
...
t_depth = swan.s616076.(varname).Depth;
tt = find(t_depth > -0.1);

[t_date,ind] = unique(swan.s616076.(varname).Date(tt));
t_data = swan.s616076.(varname).Data(tt(ind));

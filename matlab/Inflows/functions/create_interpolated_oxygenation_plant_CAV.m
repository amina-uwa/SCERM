function create_interpolated_oxygenation_plant_CAV(oxy,headers,datearray)
...
for i = 1:length(headers)
    switch headers{i}
        case 'Flow'
            Flow = interp1(oxy.CAV.mdate(~isnan(Flow_t)),Flow_t(~isnan(Flow_t)),datearray);
            Flow(isnan(Flow)) = 0;
        case 'Oxy'
            Oxy = interp1(oxy.CAV.mdate(~isnan(Oxy_T)),Oxy_T(~isnan(Oxy_T)),datearray);
            Oxy(isnan(Oxy)) = 0;
        case 'Sal'
...
filename = 'Cav_Oxygenation_Plant.csv';
